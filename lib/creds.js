...
class Credentials {
    constructor (argv, opts) {
        ...
        const validate = ajv.compile(require('../data/rc_schema.json'));
        ...
        if (opts.template && argv.template) {
            this.name = path.parse(argv.template).name.replace(/\.template/, '') + '-';
        ...
        if (argv.name) {
            ...
            this.name = this.name + this.stack;
        }
    ...
    static dot_deploy() {
        ...
        const validate = ajv.compile(require('../data/schema.json'));
